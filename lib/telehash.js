(function () {
	...
	var STATE = exports.STATE = require("./state.js");
	var MODE = exports.MODE = require("./mode.js");
	var TIMERS = exports.TIMERS = require("./timer.js");
	...
	*/
	...
	function connectResponseListener(connect) {
		if (self.mode === MODE.ANNOUNCER) return;
		var end = new hlib.Hash(self.me.ipp); //end we are tapping for
		var hash = new hlib.Hash(connect); //where we will .tap
		var rule = {
			'is': {
				'+end': end.toString()
			},
		...
		var listener = {
			...
			cb: undefined,
			far: true,
			_connect: connect //keep reference for when we need to update this listener
		};
		return listener;
	}
	...
	function listenLoop() {
		...
		for (var conn in connectors) {
			if (connectors[conn].listener && connectors[conn].listener.off === false) {
				active_listeners.push(conn.listener);
			}
		}
