...
function doSee(s, see) {
	...
	see.forEach(function (ipp) {
		...
		//add switch to network, will be ping'ed on next scan interval (will it still be healthy by then?)
		getSwitch(ipp, {
			via: s.ipp
		}).visible = true;
	});
...
Switch.prototype.drop = function () {
	...
	if (this.healthy() && this._line) this.send({
		_br: -10000
	});
