...
function getSelf(arg, onInitialised) {
	...
	//socket must already be bound and
	if (self.socket) {
	...
	udplib.createSocket(self.udplib ? self.udplib : "enet", incomingDgram, self.port ? parseInt(self.port) :
		0, self.interface,
		function (err, socket) {
			if (err) {
				doShutdown();
	...
	function initialiseSelf(socket) {
		...
		self.scanInterval = setInterval(scan, 25000); // every 25sec, so that it runs 2x in <60 (if behind a NAT to keep mappings alive)
...
function doBroadcast() {
	...
	self.broadcastInterval = setInterval(sendBroadcastTelex, 10000); //broadcast every 10 seconds looking for new telehash switches on the LAN
	sendBroadcastTelex();
}
...
function handleSeedTelex(telex, from, len) {
	...
	if (!self.me && telex._to) {
		...
		setTimeout(function () {
			...
			self.state = STATE.online;
			if (self.log) self.log("going online", "nat:", self.nat, "snat:", self.snat, "visible:",
				self.me
				.visible, "mode:",
				self.mode);
...
function doConnect(end_name) {
	if (!self.me) return;
	if (self.state != STATE.online) return;
	if (self.mode === MODE.ANNOUNCER) noResponse = true;
		...
		send: function (message, callback, timeOut) {
			...
			if (self && callback && self.mode != MODE.ANNOUNCER) {
				responseHandlers[guid] = {
					...
					timeout: timeOut ? Date.now() + (timeOut * 1000) : Date.now() + (10 * 1000),
...
function doShutdown() {
	...
	resetIdentity();
	if (self.onStatusChange) self.onStatusChange("shutdown"); // a callback still waiting?!
	self = undefined;
}
...
function scan() {
	...
	all.forEach(function (s) {
		if (s.self) return;
		/*console.error("switch:", s.ipp, "visible:", s.visible, "line:", s.line, "misses:", s.misses,
			"healthy:", s.healthy());*/
	});
	...
	if (!self.broadcastMode) {
		...
		if (all.length <= 1 && !self.seed) { //We probably lost our internet connection at this point.. or maybe
