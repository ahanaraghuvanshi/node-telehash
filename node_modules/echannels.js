...
function activateChannel(ipp, enet_peer, channel) {
    ...
    channels[ipp] = {        
        ...
        send:function(buffer,subchannel){
            //try{
               enet_peer.send(subchannel?subchannel:0, new enet.Packet(buffer,enet.Packet.FLAG_RELIABLE) );
            //}catch(e){}
        },
...
function enet_on_disconnect(enet_peer,data){
    console.error("ENET_ON_DISCONNECT");
    var ip = enet_peer.address().address();
    var port = enet_peer.address().port();    
    var ipp = ip+":"+port;
...
function handleConnect(conn, callback) {
    ...
    var randID = Math.round(Math.random()*65535);    
        ...
        if(!self.snat){
            ...
            setTimeout(function(){
                if (!peers[message.ipp]) {
                    ...
                    self.server.host.connect(new enet.Address(conn.message.ipp), 2, parseInt(randID));
                }                
            },1000);
    ...
    if( util.IP(conn.message.ipp)==self.me.ip && conn.message.localipp) {         
        ...
        console.log("Sending our LOCAL IP",addr);
...
function handleResponse(message, callback) {         
    ...
    if( message.status == "LOCAL"){        
        ...
        if (!peers[message.ipp]) {            
            ...
            console.log("LOCAL PEER:",message.ipp);
            self.server.host.connect(new enet.Address(message.ipp), 2, parseInt(message.id));
        }
        return;
    }
